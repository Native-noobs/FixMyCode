...
const ModalForLesson: FC<LessonModalPros> = ({ isOpen, setIsOpen }) => {
    if (!isOpen) {
        return
    }
        ...
        <div className="w-[20%] relative z-10 rounded-2xl p-10 h-[30%] bg-[rgba(81,82,127,0.6)] flex flex-col justify-between">
            <div>
                ...
                <div className="mt-10 flex justify-center">
                    <input type="text" className="rounded-md w-full h-10 pl-2 text-slate-900 focus:outline-sky-900" placeholder="Enter topic name" onChange={(e) => {
                        setLesson(e.target.value)
                    }} />
                </div>
            </div>
            <div className="flex justify-between">
                <button className="md:w-24 w-full bg-sky-500 hover:bg-sky-600 active:bg-sky-300 rounded-xl h-10 duration-200 disabled:bg-[rgba(125,211,252,0.4)] flex items-center justify-center gap-2" onClick={handleCreateTopic} disabled={loading}>
                    <CgSpinner
                        className={`animate-spin duration-500 ${!loading && 'hidden'}`}
                    />Save</button>
                <button onClick={setIsOpen} className="md:w-24 w-full border-2 hover:bg-slate-700 rounded-xl active:bg-slate-500 h-10 duration-150">Cancel</button>
            </div>
        </div>
