...
export default async function handler(
  ...
  if (req.method === 'GET') {
    ...
    try {
      const minify = await axios.post("https://www.toptal.com/developers/javascript-minifier/api/raw", { input: req.body.code }, {
        headers: {
    ...
    homework?.testCases.map(async (e, i) => {
      execute(data, e.input, req.body.code).then(resl => {
        if (typeof resl === 'string') {
          const checkCode = resl.replace(/\n/g, '').replace(/\s/g, '').replace(/'/g, '"')
          result = [...result, {
  ...
  return new Promise((resolve, reject) => {
    exec(
      ...
      (error, stdout, stderr) => {
        error && reject({
          error: error,
          stderr: code + stderr?.split('[eval]:1')[1].split(`console.log(${input})`)[1]
        });
